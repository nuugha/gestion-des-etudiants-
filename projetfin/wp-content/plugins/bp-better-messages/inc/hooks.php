...
if ( !class_exists( 'Better_Messages_Hooks' ) ):
    ...
    class Better_Messages_Hooks
    {
        ...
        public function bm_on_deleted_user( $user_id ){
            ...
            if( count( $messages ) > 0 ) {
                foreach ($messages as $message) {
                    Better_Messages()->functions->delete_message($message->id, $message->thread_id, false, 'replace');
        ...
        public function redirect_unlogged()
        {
            if( Better_Messages()->guests->guest_access_enabled() ) return;

            $page_id = Better_Messages()->settings['chatPage'];

            if( is_numeric( $page_id ) && (int) $page_id > 0 ) {
                if ( ! is_user_logged_in() && is_page( $page_id ) ) {
                    $link = add_query_arg('redirect', urlencode( get_permalink($page_id) ), apply_filters( 'better_messages_login_url', wp_login_url( add_query_arg([]) ) ) );
                    wp_redirect( $link );
                    exit;
                }
            }
        ...
        public function new_replies_rate_limit( &$args, &$errors ){
            ...
            foreach( $user_roles as $user_role ){
                ...
                if( isset($rate_limit_reply[$user_role]) ) {
                    ...
                    $since = Better_Messages()->functions->get_microtime() - $timeAgo * 10000;
            ...
            if(class_exists('WooCommerce')){ ?>
                jQuery(document).on('bp-better-messages-update-unread', function( event ) {
                    ...
                    if ( ! element ) {
                        ...
                        var parentElement = document.querySelector('.woocommerce-MyAccount-navigation-link--bp-messages a');
